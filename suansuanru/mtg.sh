...
#=================================================
#	System Required: CentOS 7+/Debian 8+/Ubuntu 14+
#	Description: MTProxy Golang v1
#	Author: CoiaPrant
#=================================================
...
file="/usr/local/mtg"
mtproxy_file="/usr/local/mtg/mtg"
mtproxy_conf="/usr/local/mtg/mtproxy.conf"
systemd_conf="/etc/systemd/system/mtg.service"
Now_ver_File="/usr/local/mtg/ver.txt"
Crontab_file="/usr/bin/crontab"
sh_url="https://www.zeroteam.top/files/mtg.sh"
...
check_sys(){
    if [[ ! -a "/usr/bin/systemctl" ]];then
        echo -e "${Error} 系统版本过旧，不支持Systemd守护程序";
        exit 1;
    fi
...
check_pid(){
    PID=$(ps -ef| grep "/usr/local/mtg/mtg"| grep -v "grep" | grep -v "init.d" |grep -v "service" |awk '{print $2}')
}

check_new_ver(){
    new_ver=$(wget -qO- https://api.github.com/repos/9seconds/mtg/releases| grep "tag_name" | grep v1 | head -n 1| awk -F ":" '{print $2}'| sed 's/\"//g;s/,//g;s/ //g')
...
Service(){
    local command="/usr/local/mtg/mtg run -b [::]:${mtp_port}";
    if [[ "${mtp_tls}" == "YES" ]]; then
        local command="${command} --cloak-port=${mtp_port}";
    fi
    [[ ! -z "${mtp_nat_ipv4}" ]] && local command="${command} -4 \"${mtp_nat_ipv4}:${mtp_port}\"";
    [[ ! -z "${mtp_nat_ipv6}" ]] && local command="${command} -6 \"[${mtp_nat_ipv6}]:${mtp_port}\"";
    [[ ! -z "${buffer_write}" ]] && local command="${command} -w ${buffer_write}";
    [[ ! -z "${buffer_read}" ]] && local command="${command} -r ${buffer_read}"
    [[ ! -z "${stats_bind}" ]] && local command="${command} --stats-bind=\"${stats_bind}\""
    [[ ! -z "${anti_replay_max_size}" ]] && local command="${command} --anti-replay-max-size=${anti_replay_max_size}";
    [[ ! -z "${multiplex_per_connection}" ]] && local command="${command} --multiplex-per-connection=${multiplex_per_connection}";
    
    if [[ "${mtp_secure}" == "YES" && "${mtp_tls}" == "NO" ]]; then
        local command="${command} dd${mtp_passwd}";
    else
        local command="${command} ${mtp_passwd}";
    fi
    
    [[ ! -z "${mtp_tag}" ]] && local command="${command} ${mtp_tag}";
...
EOF
    ...
    systemctl enable mtg -q;
    echo -e "${Info} MTProxy服务 更新完成!"
}
...
Install(){
    ...
    check_new_ver
    Download
    echo -e "${Info} 开始下载/安装 服务脚本(Systemd)..."
    Service
...
crontab_monitor(){
    ...
    check_pid
    if [[ -z ${PID} ]]; then
        echo -e "${Error} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] 检测到 MTProxy服务端 未运行 , 开始启动..."
        systemctl start mtg
        check_pid
        if [[ -z ${PID} ]]; then
            echo -e "${Error} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] MTProxy服务端 启动失败..."
        else
            echo -e "${Info} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] MTProxy服务端 启动成功..."
        fi
    else
        echo -e "${Info} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] MTProxy服务端 进程运行正常..."
    fi
}
Set_crontab_monitorip(){
    check_crontab_installed_status
...
crontab_monitorip(){
    ...
    if [[ "${ipv4}" != "IPv4_Error" ]]; then
        if [[ "${ipv4}" != "${nat_ipv4}" ]]; then
            echo -e "${Info} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] 检测到 服务器外网IPv4变更[旧: ${nat_ipv4}，新: ${ipv4}], 开始重新配置并准备重启服务端..."
            monitorip_yn="YES"
            mtp_nat_ipv4=${ipv4}
        fi
    else
        echo -e "${Error} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] 服务器外网IPv4获取失败..."
        mtp_nat_ipv4=${nat_ipv4}
    fi
    if [[ "${ipv6}" != "IPv6_Error" ]]; then
        if [[ "${ipv6}" != "${nat_ipv6}" ]]; then
            echo -e "${Info} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] 检测到 服务器外网IPv6变更[旧: ${nat_ipv6}，新: ${ipv6}], 开始重新配置并准备重启服务端..."
            monitorip_yn="YES"
            mtp_nat_ipv6=${ipv6}
        fi
    else
        echo -e "${Error} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] 服务器外网IPv6获取失败..."
        mtp_nat_ipv6=${nat_ipv6}
    fi
...
Update_Shell(){
    sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 ${sh_url}|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
    [[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到服务器 !" && exit 0
    wget -N --no-check-certificate ${sh_url} && chmod +x mtproxy.sh
